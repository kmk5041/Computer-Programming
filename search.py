...
def get_highlight(query, sentence):
    ...
    seg=query.split()
    for q in seg:
        s=s.replace(q,'<font color="blue"><b>'+q+'</b></font>')
    return s
...
def search( inverted_indexing, sentences, query):
    ...
    seg=query.split()
    a=set()
    if(seg[0] in inverted_indexing):
        a=a|inverted_indexing[seg[0]]
    if(len(seg)>1):
        for i in range(1,len(seg)):
            if(seg[i] in inverted_indexing):
                a=a&inverted_indexing[seg[i]]
