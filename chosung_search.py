...
def chosung_indexing(cho_indexing, filename):
    ...
    with open(filename) as fin:
        for line in fin.readlines():
            ...
            if(str2chostr(spl[0]) in cho_indexing):
                cho_indexing[str2chostr(spl[0])].append(spl)
            else:
                cho_indexing[str2chostr(spl[0])]=[spl]
    for k,v in cho_indexing.items():
        cho_indexing[k].sort(key=lambda x: (-x[1], x[0]))
